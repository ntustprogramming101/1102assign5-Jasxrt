...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < clockX.length; i++) {
      ...
      if (clockX[i] + SOIL_SIZE > playerX
        && clockX[i] < playerX + SOIL_SIZE
        && clockY[i] + SOIL_SIZE > playerY
        && clockY[i] < playerY + SOIL_SIZE) {
...
String convertFramesToTimeString(int frames) {
  int second;
  second = floor(frames/60);
  int a=floor(second/60);
  int b=second%60;

  return nf(a, 2)+":"+ nf(b, 2);
}
